...
const ResetPassword = () => {
  ...
  useEffect(() => {
    const validateToken = async () => {
      try {
        const response = await axios.get(`https://swe-xy4l.onrender.com/reset-password/${decodedToken}`);
  ...
  const onSubmit = async (data) => {
    try {
      const response = await axios.post(`https://swe-xy4l.onrender.com/reset-password/${decodedToken}`, data);
      if (response.status === 200) {
