...
const ResetPassword = () => {
  ...
  useEffect(() => {
    const validateToken = async () => {
      try {
        const response = await axios.get(`https://happymed.work.gd/reset-password/${decodedToken}`);
  ...
  const onSubmit = async (data) => {
    try {
      const response = await axios.post(`https://happymed.work.gd/reset-password/${decodedToken}`, data);
      if (response.status === 200) {
